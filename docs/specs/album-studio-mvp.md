...
- Draft estimates are informative and do not lock price. Order placement includes `acceptedPrice`: Order total, per-unit total, and deterministic per-unit breakdown with selected pricing IDs. Server always reloads authoritative product, pricing, requirements, and availability, then reevaluates. Invalid current evaluation returns `CONFIGURATION_INVALID` with current product definition. Any valid current quote differing from `acceptedPrice` returns `PRICE_CHANGED` with accepted/current quotes and current product definition. Resubmission with current quote constitutes explicit acceptance, subject to another fresh server reload and reevaluation.
...
- [x] Slice 6: Server-reconciled Order placement
  - Acceptance: Checkout checkpoints current dirty editor snapshot before placement; checkout independently reloads current authoritative product, pricing, requirements, and availability and reevaluates regardless of Draft summary; invalid configuration or availability returns `CONFIGURATION_INVALID` with current product definition and points to affected group; any valid current quote differing from `acceptedPrice` returns `PRICE_CHANGED` with accepted/current quotes and current product definition; resubmission with current quote explicitly accepts it subject to fresh reevaluation; one transaction converts Draft into exactly one immutable Order; repeated submission is idempotent.

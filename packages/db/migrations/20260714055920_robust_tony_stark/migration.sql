...
ALTER TABLE "offline_payment" ADD CONSTRAINT "offline_payment_reversal_of_id_offline_payment_id_fkey" FOREIGN KEY ("reversal_of_id") REFERENCES "offline_payment"("id") ON DELETE RESTRICT;--> statement-breakpoint
ALTER TABLE "offline_payment" ADD CONSTRAINT "offline_payment_order_organization_fkey" FOREIGN KEY ("order_id","organization_id") REFERENCES "customer_order"("id","organization_id") ON DELETE RESTRICT;
